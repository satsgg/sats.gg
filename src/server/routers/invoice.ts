...
export const invoiceRouter = t.router({
  createInvoice: t.procedure
    ...
    .query(async ({ ctx, input }) => {
      // tx is to self if both input values are undefined
      // need better way to handle these
      if (!input.toUserId || !input.toUserName) {
        input.toUserId = ctx.user.id
        input.toUserName = ctx.user.userName
      }
      // } else {
      //   throw new TRPCError({ code: 'BAD_REQUEST', message: 'missing input'})
      // }
      ...
      const description = `${input.amount} sats for @${input.toUserName} on ${process.env.DOMAIN} at ${format(
        new Date(),
        'dd.MM.yyyy hh:mm:ss',
      )}`

      const balance = await userBalance(prisma, input.toUserId)

      if (balance >= PER_USER_BALANCE_CAP) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'user balance too high' })
      }

      console.log('toUserId', input.toUserId, 'toUserName', input.toUserName)

      const descriptionHash = lnurlPayDescriptionHash(description)
      try {
        const lndInvoice = await createInvoice({
          lnd,
          tokens: balance + input.amount > PER_USER_BALANCE_CAP ? PER_USER_BALANCE_CAP - balance : input.amount,
