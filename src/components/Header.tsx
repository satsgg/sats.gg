        ...
        <div className={'flex flex-row justify-center justify-between'}>
          {user.profileImage ? (
            <button onClick={openEditUser} id={'button-edit-profile'}>
              <img className="mr-2 h-10 w-10" src={user.profileImage} alt={`profile image of ${user.userName}`} />
            </button>
          ) : (
            <svg
          ...
          <div className={'grow'}>
            ...
            <IconPropertyDisplay identifier={'createdAt'} value={format(user.createdAt ?? 0, standardDateFormat)}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
